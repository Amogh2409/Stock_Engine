...
| ~~Illiquidity (Amihud)~~ | Compensation for immediacy. | registered `b20e86c`, run, **FAILED**, frozen |
| **Long-horizon illiquidity premium** | Illiquidity compensation accrues over quarters rather than a month. | **QUARANTINED / DO NOT TEST — see below** |

### Long-horizon illiquidity premium — quarantined at birth

**Origin: generated BY the failed pre-registered Amihud study**, not
independently. Recorded now, while the provenance is fresh, so that nobody in
six months remembers it as an idea they simply had.

Diagnostics that produced it, from `b20e86c`:

| horizon | IC | floor | p (Bonf) | IC>0 |
|---|--:|--:|--:|--:|
| 3M | +0.0823 | 0.052 | 0.000 | 76% |
| 6M | +0.1183 | 0.069 | 0.001 | 83% |
| 12M | +0.1562 | 0.115 | 0.023 | 97% |

**Major unresolved confound: point-in-time size exposure is unavailable.** The
most-illiquid decile is plausibly the small-cap decile, and nothing in the
current data separates those two readings.

**Status: QUARANTINED / DO NOT TEST.**

**Unlock condition:** enough point-in-time market-capitalisation history exists
to measure size exposure *without backfilling current values*. Collection of
that field starts now, so the clock starts now.

The question it would eventually ask, written once so it cannot drift:

> Does illiquidity carry longer-horizon information **after controlling for
> point-in-time size**, or is the pattern primarily small-cap exposure?

That is a better future experiment than MAX or another technical variation --
which is exactly why it must wait for data that can answer it rather than be
run early on data that cannot.
...
| BAB | quarantined |
| Long-horizon illiquidity | **quarantined at birth** -- generated by the failed Amihud study; blocked on PIT market cap |
...
# PHASE DECLARATION — 2026-09-16

```
PRICE-ONLY ALPHA RESEARCH
Status:       PAUSED
Budget used:  2 / 3
Reason:       the strongest remaining hypothesis is blocked by missing
              point-in-time size data
Last slot:    PRESERVED
```

Four families measured, four frozen:

| family | result |
|---|---|
| trend / momentum / relStrength / volume | FAIL |
| low / idiosyncratic volatility | FAIL |
| short-term reversal | FAIL |
| Amihud illiquidity | FAIL |

**The budget is a maximum, not a quota.** Slot 3 is preserved deliberately, not
left over. Spending it on MAX — the only screened-distinct candidate left, and
one entangled at ~0.53 with a family that already failed — would be worse
research than waiting, because the question actually worth asking is already
identified and is blocked on data rather than on ideas.

## Active work

```
[x] monthly PIT fundamentals        collect_pit.py, launchd, month-idempotent
[x] monthly PIT market cap          seeded 2026-09-16, 100/100 populated
[x] archive / data-quality infra    archive_health, diagnose_price_panel,
                                    amihud_readiness, contract audit
[x] no new scoring changes          scoring hash guarded; four families frozen
```

None of this consumes research budget, because none of it consults a forward
outcome. It improves the credibility of every future result instead of
generating another signal.

## On how the Amihud result is described

The headline is **FAIL under the pre-registered decision rule.** The margin —
0.0435 against a floor of 0.0446 — is recorded as metadata inside the
registration and must not become the headline. "97.5% of the threshold" turns
into "basically worked" with very little help, and the decile spread and
positive half-sample ICs are diagnostics, not grounds to override condition 2.

## When this un-pauses

Not on a new idea. On data: enough PIT market-cap history to ask whether
illiquidity carries longer-horizon information after controlling for size, or
whether the pattern is small-cap exposure. `archive_health.py` reports the
